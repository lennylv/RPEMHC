...
class hla_ii_mult_cnn(nn.Module):
    def __init__(self, args, mode=None):
        super(hla_ii_mult_cnn, self).__init__()
        # mult_id
        self.mult_emb = nn.Embedding(441 + 1, 128)
        self.mult_id_step1 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=256, kernel_size=(34, 1), padding='valid'), nn.BatchNorm2d(256),
            nn.ReLU(),
        )
        ...
        self.mult_id_step2_2 = nn.Sequential(
            nn.Conv1d(in_channels=256, out_channels=128, kernel_size=3, padding='same'), nn.BatchNorm1d(128), nn.ReLU(),
            nn.Dropout(0.15)
        )

        self.mult_id_step2_3 = nn.Sequential(
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=5, padding='same'), nn.BatchNorm1d(256), nn.ReLU(),
            nn.Dropout(0.2)
        )
        ...
        self.mult_id_step3_4 = nn.Sequential(
            nn.Conv1d(in_channels=256, out_channels=1024, kernel_size=7, padding='same'), nn.BatchNorm1d(1024),
            nn.ReLU()
        )
    ...
    def forward(self, mult_id, pep_mask_len, targets):
        ...
        mult_id = mult_id.reshape([mult_id.shape[0], 34, 20, -1]).permute([0, 3, 1, 2])
        ...
        # masks_id = torch.repeat_interleave(masks_id, mult_id.shape[2], dim=2).bool()
        # masks_id = torch.cast(masks_id, dtype='bool')
        mult_id,_ = mult_id.masked_fill(~masks_id, -0.0000000000000000000000000000000001).max(dim=1)

        output = F.relu(self.fc(mult_id))

        return output, targets

    def masked_fill(self, x, mask, value):
        y = torch.full(x.shape, value)
